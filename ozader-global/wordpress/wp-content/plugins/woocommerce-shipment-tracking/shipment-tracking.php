				...
				$this->providers = array(
					...
					'Chile' => array(
						...
						'Chilexpress'
							...
							=> 'https://www.chilexpress.cl/Views/ChilexpressCL/Resultado-busqueda.aspx?DATA=%1$s',
...
ericSiteIdent=',
			...
			function display_tracking_info( $order_id, $for_email = false ) {
				...
				if ( $date_shipped )
					...
					$date_shipped = ' ' . sprintf( __( 'dia %s', 'wc_shipment_tracking' ), date_i18n( __( 'l F Y', 'wc_shipment_tracking'), $date_shipped ) );
				...
				if ( $tracking_provider ) {
					...
					if ( $link_format ) {
						...
						if ( $for_email ) {
							...
							$tracking_link = sprintf( __('Haga clic aquí para realizar el seguimiento de su compra', 'wc_shipment_tracking') . ': <a href="%s">%s</a>', $link, $link );

						} else {

							$tracking_link = sprintf( '<a href="%s">' . __('', 'wc_shipment_tracking') . '.</a>', $link, $link );
					...
					echo wpautop( sprintf( __('Su pedido fue enviado %s%s. El número de seguimiento es %s. %s', 'wc_shipment_tracking'), $date_shipped, $tracking_provider, $tracking_number, $tracking_link ) );
					...
					if ( $custom_tracking_link ) {
						...
						$tracking_link = sprintf( '<a href="%s">' . __('Haga clic aquí para realizar el seguimiento de su compra', 'wc_shipment_tracking') . '.</a>', $custom_tracking_link );
